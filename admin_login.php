...
if(!empty($_POST)) {
    ...
    $truyvan = "SELECT * FROM USERS WHERE ID_USER='".$username."' AND PASS_USER='".$pass."' LIMIT 1";
    $ketqua = mysqli_query($conn,$truyvan);
    $demdong = mysqli_num_rows($ketqua);
    if($demdong >=1){
        setcookie("ID_USER", $username, time() + 3600, "/","", false, true);
...
<head>
    ...
    <title>Agric Shop - Đăng nhập trang quản trị</title>
                    ...
                    <div class="card card-signup">
                        <form class="form" method="POST" action="">
                            <div class="card-header card-header-success text-center">
                                <h4 class="card-title">Đăng nhập trang quản trị</h4>
                            </div>
